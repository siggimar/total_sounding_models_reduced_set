...
def format_jointplot_ax( ax_main, ax_top, ax_side, xlims, ylims, logx, logy, rc, rc_lims, detailed ):
    ...
    if rc[1]==0: ax_main.set_ylabel( 'std(' + r'$F_{DT}$' + ') (kN)', fontsize=f_size_axlabels )


def calc_perc_elev( X, Y, Z, perc ):
    ''' 
        calculates the elevation (surface contour) that countains 50% of the PDF volume
...
if __name__=='__main__':
    ...
    for i in range(1,2):
        summary(n=i, f_name='fig_8_simplified_03_SBT_chart.png', detailed=True, N=500) # 'all_classes_' + str(i) + '.png', all_classes_' + str(i) + '.png # N > 300 useable - 500 used in paper.